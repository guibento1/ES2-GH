...
# ---------------------------------------------------------------------------
# TU-81 a TU-83 — Tipo de plano
# ---------------------------------------------------------------------------

@pytest.mark.parametrize(
    "test_id, plan_type, esperado",
    [
        ("TU-81", "regular",    "ok"),    # PE: classe válida
        ("TU-82", "emergência", "ok"),    # PE: classe válida
        ("TU-83", "invalido",   "erro"),  # PE: classe inválida
    ],
)
def test_tipo_plano(test_id, plan_type, esperado):
    """PE: tipo de plano — regular e emergência aceites; tipo desconhecido rejeitado (TU-81 a TU-83)."""
...
# ---------------------------------------------------------------------------
# TU-84 a TU-88 — Duração do ciclo [1, 365] dias
# ---------------------------------------------------------------------------

@pytest.mark.parametrize(
    "test_id, days, esperado",
    [
        ("TU-84",   0, "erro"),   # VL: abaixo do limite inferior (0 < 1)
        ("TU-85",   1, "ok"),     # VL: limite inferior exacto
        ("TU-86",  90, "ok"),     # VL: valor nominal interior
        ("TU-87", 365, "ok"),     # VL: limite superior exacto
        ("TU-88", 366, "erro"),   # VL: acima do limite superior (366 > 365)
    ],
)
